...
class VehicleCommandServiceTest {
    ...
    @Test
    void testUpdateVehicleLocationSuccess() {
        when(vehicleRepository.findById("VEH-001")).thenReturn(Optional.of(testVehicle));

        LocationPoint newLocation = new LocationPoint(40.7128, -74.0060, 30.0f, testTime);
        commandService.updateVehicleLocation("VEH-001", newLocation);
    ...
    @Test
    void testUpdateVehicleLocationWithInvalidLocation() {
        when(vehicleRepository.findById("VEH-001")).thenReturn(Optional.of(testVehicle));

        LocationPoint invalidLocation = new LocationPoint(100.0, -74.0060, 30.0f, testTime);

        VehicleException exception = assertThrows(VehicleException.class,
            () -> commandService.updateVehicleLocation("VEH-001", invalidLocation));
    ...
    @Test
    void testUpdateVehicleLocationVehicleNotFound() {
        when(vehicleRepository.findById("VEH-999")).thenReturn(Optional.empty());

        LocationPoint location = new LocationPoint(40.7128, -74.0060, 30.0f, testTime);

        VehicleException exception = assertThrows(VehicleException.class,
            () -> commandService.updateVehicleLocation("VEH-999", location));
    ...
    void testRepositoryInteraction() {
        // Verify that repository methods are called correctly
        when(vehicleRepository.findById("VEH-001")).thenReturn(Optional.of(testVehicle));

        LocationPoint location = new LocationPoint(40.7128, -74.0060, 0.0f, testTime);
        commandService.updateVehicleLocation("VEH-001", location);
    ...
    void testEventPublisherInteraction() {
        ...
        verify(eventPublisher, times(1)).publish(any(com.jjenus.tracker.shared.pubsub.DomainEvent.class));
    }
}
